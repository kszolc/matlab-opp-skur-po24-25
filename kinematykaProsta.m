function kinematykaProsta()
    ...
    start_point = getPoint(start_z, wysokosc,promien_wew,promien_zew, 'Punkt Początkowy');

    % Pobranie punktu końcowego
    end_point = getPoint(start_z, wysokosc,promien_wew,promien_zew, 'Punkt Końcowy');
...
function values = getPoint(z_min, z_max, r_wewn, r_zew, nazwa)
    % Przykładowe zakresy dopuszczalnych wartości (dostosuj według potrzeb)
    q3_range = r_zew - r_wewn;
    q2_range = z_max;
    q1_range = 360;
    
valid = false;
while ~valid
    % Pobranie wartości X, Y i Z w jednym oknie
    prompt = {
        'Przesunięcie Q1 (0-360deg):', ...
        sprintf('Przesunięcie Q2 (0 - %.2f):', q2_range), ...
        sprintf('Przesunięcie Q3 (0 - %.2f):', q3_range)
    };
    defaultAnswers = {'0', '0', '0'}; % Domyślne wartości
    answers = inputdlg(prompt, nazwa, [1, 50; 1, 50; 1, 50], defaultAnswers);
    
    if isempty(answers)
        errordlg('Nie wprowadzono wartości!', 'Błąd');
        continue; 
    end

        % Konwersja wprowadzonych wartości
    Q1Val = str2double(answers{1});
    Q2Val = str2double(answers{2});
    Q3Val = str2double(answers{3});
        
        % Sprawdzenie poprawności i zakresów
        if isnan(Q1Val) || isnan(Q2Val) || isnan(Q3Val)
            errordlg('Musisz wprowadzić wartości liczbowe!', 'Błąd');
            continue;
        elseif Q1Val < 0 || Q1Val > q1_range
            errordlg('Wartość q1 musi być w zakresie 0 - 360!', 'Błąd');
        elseif Q2Val < 0 || Q2Val > q2_range
            errordlg(sprintf('Wartość q2 musi być w zakresie 0 - %.2f!', q2_range), 'Błąd');
        elseif Q3Val < 0 || Q3Val > q3_range
            errordlg(sprintf('Wartość Z musi być w zakresie 0 - %.2f]!', q3_range), 'Błąd');
        else
            % Wszystkie wartości poprawne
            valid = true;
        end
end


% Przekształcenie wartości Q1, Q2, Q3 na współrzędne (x, y, z)

% Konwersja Q1 z stopni na radiany
q1_rad = deg2rad(Q1Val);

% Obliczenie pozycji w przestrzeni kartezjańskiej
x = (r_wewn + Q3Val) * cos(q1_rad); % Pozycja x
y = (r_wewn + Q3Val) * sin(q1_rad); % Pozycja y
z = z_min + Q2Val;                  % Pozycja z

% Zwracanie końcowego wektora pozycji
values = [x, y, z];
